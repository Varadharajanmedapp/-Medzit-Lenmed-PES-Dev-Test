...
class Email_model extends CI_model {
    ...
    public function send_verification_email($email,$otp)
    {
        $admin_mail = $this->email_model->getEmailSettings();
        $admin_email = $admin_mail->admin_email;
        if($admin_email==null){
            $admin_email = "admin.email@india.medzit.com";
        }
        include_once(ROOTPATH.'/includes/mail/phpmailer/class.phpmailer.php');
        include_once(ROOTPATH.'/includes/mail/phpmailer/class.smtp.php');
        include_once(ROOTPATH.'/includes/mail/phpmailer/PHPMailerAutoload.php');
         $subject= "To Verify Your Email in Medzit";
        $message = "The OTP to verify your account in Medzit is ".$otp;
        $mail = new PHPMailer;
        $mail->IsSendmail();
        $mail->isSMTP();
        $mail->SMTPAuth = true;
    ...
    public function send_support_email($email,$body)
    {
        $admin_mail = $this->email_model->getEmailSettings();
        $admin_email = "admin.email@india.medzit.com";
        include_once(ROOTPATH.'/includes/mail/phpmailer/class.phpmailer.php');
