...
// } 
    ...
    function getPayment() {
        ...
        foreach ($data['payments'] as $payment) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Accountant'))) {
                $options1 = ' <a class="btn btn-info btn-xs editbutton" title="' . lang('edit') . '" href="finance/editPayment?id=' . $payment->id . '"> <img alt="" src="uploads/edit-bttn-icon.png" style="height: 20px;"> ' . lang('edit') . '</a>';
            }
            ...
            if ($this->ion_auth->in_group(array('admin', 'Accountant'))) {
                $options3 = '<a class="btn btn-info btn-xs delete_button" title="' . lang('delete') . '" href="finance/delete?id=' . $payment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><img alt="" src="uploads/trash-bttn-icon.png" style="height: 20px;">  ' . lang('delete') . '</a>';
            }
           ...
           $total_gross_total += $payment->gross_total;
           $total_payment += $this->finance_model->getDepositAmountByPaymentId($payment->id);
        }
        $info[] = array(
            '',
            'Total',
            '',
            '',
            $settings->currency . '' . $total_amount,
            $settings->currency . '' . $total_discount,
            $settings->currency . '' . $total_gross_total,
            $settings->currency . '' . $total_payment,
            '',
            '',
            '' . ' ' . '' . ' ' . '' . ' ' . '',
                //  $options2
    ...
    function getExpense() {
        ...
        foreach ($data['expenses'] as $expense) {
            ...
            if ($this->ion_auth->in_group(array('admin'))) {
                $options1 = ' <a class="btn btn-info btn-xs editbutton" title="' . lang('edit') . '" href="finance/editExpense?id=' . $expense->id . '"><img alt="" src="uploads/edit-bttn-icon.png" style="height: 20px;">Edit</a>';
            }

            $options2 = '<a class="btn btn-info btn-xs invoicebutton" title="' . lang('invoice') . '" style="color: #fff;" href="finance/expenseInvoice?id=' . $expense->id . '"><i class="fa fa-file-invoice"></i> Invoices </a>';
            //$options4 = '<a class="btn btn-info btn-xs invoicebutton" title="' . lang('print') . '" style="color: #fff;" href="finance/printInvoice?id=' . $payment->id . '"target="_blank"> <i class="fa fa-print"></i> ' . lang('print') . '</a>';
            if ($this->ion_auth->in_group(array('admin'))) {
                $options3 = '<a class="btn btn-info btn-xs delete_button" title="' . lang('delete') . '" href="finance/deleteExpense?id=' . $expense->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><img alt="" src="uploads/trash-bttn-icon.png" style="height: 20px;">Delete</a>';
            }
    ...
    public function get_payment_details()
    {
        ...
        $from_date = strtotime(strval($from));
        $to_date = strtotime(strval($to));
        $paid_from = $this->input->post('check_type');
        $result = $this->finance_model->get_payment_detail($from_date, $to_date, $paid_from);
        return $result;
    }
