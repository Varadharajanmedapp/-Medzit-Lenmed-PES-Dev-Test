...
class Pharmacy extends MX_Controller {
    ...
    public function addPaymentView() {
        ...
        $data['settings'] = $this->settings_model->getSettings();
        $data['medicines'] = $this->medicine_model->getMedicine();
        $data['patient_ids']=$this->pharmacy_model->getPatientId();
    ...
    public function appointmentbyid()
    {
    $patient_id = $this->input->post('patient_id');
    $appointment=$this->pharmacy_model->getAppointmentsByPatientId($patient_id);
    ...
    public function addPayment() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($discount_type == 'flat') {
                ...
                $gross_total = intval($sub_total) - intval($flat_discount);
            } else {
    ...
    function getPaymentList() {
        ...
        foreach ($data['payments'] as $payment) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Pharmacist'))) {
                $option1 = '<a class="btn btn-info btn-xs editbutton" href="finance/pharmacy/editPayment?id=' . $payment->id . '"><img alt="" src="uploads/edit-bttn-icon.png" style=" height: 17px;">' . lang('edit') . '</a>';
            }
            if ($this->ion_auth->in_group('admin')) {
                $option2 = '<a class="btn btn-info btn-xs btn_width delete_button" href="finance/pharmacy/delete?id=' . $payment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><img alt="" src="uploads/trash-bttn-icon.png" style=" height: 20px;">Delete</a>';
            }
