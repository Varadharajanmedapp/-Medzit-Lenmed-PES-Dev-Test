            ...
            <div class="">
                <div class="adv-table editable-table ">
                    <div class="clearfix">
                        ...
                        <form role="form" class="clearfix pos form1 pos-form2"  id="editPaymentForm" action="finance/pharmacy/addPayment" method="post" enctype="multipart/form-data" >
                            ...
                            <div class="col-md-12 payment right-six">
                                    <div class="col-md-3 payment_label"> 
                                        <label for="exampleInputEmail1"> <?php echo lang('patient_id'); ?></label>
                                    </div>
                                   
                                    <div class="col-md-9"> 
                                        <select id="patient_id" class="form-control m-bot15" name="patient_id" >
                                      
                                        <?php if(!$payment->patient=="") {?>
                                        
                                        <option value="<?php echo $payment->patient;?>" ><?php echo $payment->patient;?></option> 
                                        
                                            <?php }  else{?>
                                            <option>Select Patient</option>                     
                                         <?php foreach($patient_ids as $patient_id) {?>
                                            <option value="<?php echo $patient_id->id;?>"><?php echo $patient_id->id;?> : <?php echo $patient_id->name;?></option>
                                            <?php }}?>  
                                        </select>
                                    </div>
                            </div>
                            ...
                            <div class="col-md-12 payment right-six">
                                    ...
                                    <div class="col-md-9"> 
                                        <select id="appointment_id" class="form-control m-bot15" name="appointment_id">
                                        <?php if(!empty($payment->appointment_id)){?>
                                            <option value="<?php echo $payment->appointment_id ?>"><?php echo $payment->appointment_id?></option>
...
<script>
	$(document).ready(function(){
		$("#patient_id").change(function(){
            ...
            var patient_arr = patient_data.split(":")
            var patient_id = parseInt(patient_arr[0]);
        
			if(patient_id!=""){
				$.ajax({
					url:"<?=base_url('finance/pharmacy/appointmentbyid')?>",
					method:"post",
                    dataType: 'json',
					data:{patient_id:patient_id},
					success:function(data){
                    html = '<option value="">Select Appointments</option>';
          
        $.each(data, function (key, val) 
        {
  
        html += "<option value='"+val.id+"'> ID:" +val.id+" / Date: "+val.add_date+"</option>";
        });
      ...
      {
        $("#appointment_id").html('<option value="">Select Appointments</option>');
      }
		});
	});
</script>