...
class Appointment extends MX_Controller {
    ...
    public function index() {
        ...
        $data['specialists'] = $this->department_model->get_med_procedure();
        $data['departments'] = $this->appointment_model->get_departments();
    ...
    public function pending() {
        ...
        $data['settings'] = $this->settings_model->getSettings();
        $this->load->view('home/dashboard', $data); // just the header file
        $this->load->view('appointment_pending', $data);
        $this->load->view('home/footer'); // just the header file
    }
    ...
    public function addNew() {
        ...
        $consult_type = $this->input->post('consult_type');
        $type = $this->input->post('specialist');
        ...
        $this->form_validation->set_rules('patient', 'Patient', 'trim|required|min_length[1]|max_length[100]|xss_clean');
         ...
         $this->form_validation->set_rules('consult_type', 'Consult Type', 'trim|required|min_length[1]|max_length[100]|xss_clean');
         ...
         $this->form_validation->set_rules('sms', 'SMS', 'trim|xss_clean');
         $this->form_validation->set_rules('status', 'Status', 'trim|xss_clean');
         $this->form_validation->set_rules('redirect', 'Redirect', 'trim|xss_clean');
         $this->form_validation->set_rules('request', 'Request', 'trim|xss_clean');
         $this->form_validation->set_rules('department', 'Department', 'trim|required|min_length[1]|max_length[100]|xss_clean');
         $this->form_validation->set_rules('patient_query', 'Patient query', 'trim|required|min_length[1]|max_length[100]|xss_clean');
         $this->form_validation->set_rules('p_email', 'Patient Email', 'trim|valid_email|max_length[100]|xss_clean');
         $this->form_validation->set_rules('specialist', 'Medical procedure', 'trim|required|min_length[1]|max_length[100]|xss_clean');
         $this->form_validation->set_rules('p_age', 'Patient Age', 'trim|numeric|xss_clean');
         $this->form_validation->set_rules('p_gender', 'Patient Gender', 'trim|xss_clean');
...
$files_count = count($_FILES['img_url']['name']);
$files = $_FILES['img_url'];

$img_urls = array(); // Initialize an array to store uploaded URLs

for ($i = 0; $i < $files_count; $i++) {
    $_FILES['img_url']['name'] = $files['name'][$i];
    $_FILES['img_url']['type'] = $files['type'][$i];
    $_FILES['img_url']['tmp_name'] = $files['tmp_name'][$i];
    $_FILES['img_url']['error'] = $files['error'][$i];
    $_FILES['img_url']['size'] = $files['size'][$i];

    $this->upload->initialize($config); // Initialize the upload library for each file

    if ($this->upload->do_upload('img_url')) {
        $path = $this->upload->data();
        $img_urls[] = base_url() . "uploads/appointment/" . $path['file_name'];
    } else {
        // Handle the case when the upload fails
        $img_urls[] = ''; // Add an empty URL to indicate upload failure
    }
}
$img_urls = implode(',', $img_urls);
            ...
            $patient_query =$this->input->post('patient_query');
            $data = array();
            $data = array(
                'patient' => $patient,
        ...
        foreach ($query as $entry) {
            ...
            if(isset($entry->time_slot)){
                ...
                if ($start_time[1] == 'AM') {
                    ...
                    $day_start_time_second = intval($start_time_second[0]) * 60 * 60 + intval($start_time_second[1]) * 60;
                } else {
                    $start_time_second = explode(':', $start_time[0]);
                ...
                if ($end_time[1] == 'AM') {
                    ...
                    $day_end_time_second = intval($end_time_second[0]) * 60 * 60 + intval($end_time_second[1]) * 60;
                } else {
                    $end_time_second = explode(':', $end_time[0]);
                    $day_end_time_second = 12 * 60 * 60 + intval($end_time_second[0]) * 60 * 60 + intval($end_time_second[1]) * 60;
                }
            }else{
                $day_start_time_second = 60 * 60;
                $day_end_time_second = 12 * 60 * 60;
            }
    ...
    function editAppointmentByJason() {
        $id = $this->input->get('id');
        $data['appointment'] = $this->appointment_model->getAppointmentById($id);
        if (empty($data['appointment']->attachment_image)) {
            $data['appointment']->attachment_image = $this->appointment_model->getPrescriptionByAppointmentId($id)->attachment_image;
        }
    ...
    function getAppointment() {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist'))) {
                ...
                $options1 = ' <a type="button" class="btn editbutton" title="' . lang('edit') . '" data-toggle = "modal" data-id="' . $appointment->id . '"> <img alt="" src="uploads/edit-bttn-icon.png" style=" height: 17px;"> ' . lang('edit') . '</a>';
            }
            ...
            $options4 = '<a class="btn invoicebutton" title="' . lang('payment') . '" style="color: #fff;" href="finance/appointmentPaymentHistory?appointment=' . $appointment->id . '"><i class="fas fa-money-bill-alt"></i> ' . lang('payment') . '</a>';

            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist'))) {
                $options5 = '<a class="btn delete_button" title="' . lang('delete') . '" href="appointment/delete?id=' . $appointment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i><img alt="" src="uploads/trash-bttn-icon.png" style=" height: 25px;"></i> ' . lang('delete') . '</a>';
            }
    ...
    function getAppoinmentList() {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            $option2 = '<a class="btn btn-info btn-xs btn_width delete_button" href="appointment/delete?id=' . $appointment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><img alt="" src="uploads/trash-bttn-icon.png"  style=" height: 20px;" > ' . lang('delete') . '</a>';
            $option1 = '<button type="button" class="btn btn-info btn-xs btn_width editbutton" data-toggle="modal" data-id="' . $appointment->id . '"><img alt="" src="uploads/edit-bttn-icon.png"  style=" height: 20px; " >  ' . lang('edit') . '</button>'; 
            ...
            if(empty($appointment->s_time)){
                $appointment_time = '<br>' .$appointment->time_slot; 
            }else{
                $appointment_time = '<br>' . $appointment->s_time . '-' . $appointment->e_time;
            }
            $info[] = array(
                $appointment->id,
                $patientname,
                ...
                $appointment->patient_query,
                date('d-m-Y', $appointment->date) . $appointment_time,
                $appointment->remarks,
                $appointment->status,
        ...
        $this->db->group_start()->where('find_in_set("'.$hospital_id.'", access)')->or_where('find_in_set("'.$hospital_id2.'", access)')->or_where('hospital_id', $this->session->userdata('hospital_id'))->group_end()->get('appointment')->num_rows();
        if (!empty($data['appointments'])) {
            $output = array(
                "draw" => intval($requestData['draw']),
                "recordsTotal" => $this->db->group_start()->where('find_in_set("'.$hospital_id.'", access)')->or_where('find_in_set("'.$hospital_id2.'", access)')->or_where('hospital_id', $this->session->userdata('hospital_id'))->group_end()->get('appointment')->num_rows(),
                "recordsFiltered" => $this->db->group_start()->where('find_in_set("'.$hospital_id.'", access)')->or_where('find_in_set("'.$hospital_id2.'", access)')->or_where('hospital_id', $this->session->userdata('hospital_id'))->group_end()->get('appointment')->num_rows(),
                "data" => $info
            );
        } else {
            $output = array(
    ...
    function getRequestedAppointmentList() {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            $option1 = '<button type="button" class="btn btn-info btn-xs btn_width editbutton" data-toggle="modal" data-id="' . $appointment->id . '"><img alt="" src="uploads/edit-bttn-icon.png" style=" height: 17px;"> ' . lang('edit') . '</button>';
    ...
    function getUpcomingAppoinmentList() {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            $option2 = '<a class="btn btn-info btn-xs btn_width delete_button" style="border-bottom:0px;" href="appointment/delete?id=' . $appointment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><img alt="" src="uploads/trash-bttn-icon.png"  style=" height: 20px;" > ' . lang('delete') . ' </a>';
            if ($appointment->date > strtotime(date('Y-m-d'))) {
                $patientdetails = $this->patient_model->getPatientById($appointment->patient);
                if (!empty($patientdetails)) {
                    $patientname = ' <a type="button" class="history" data-toggle = "modal" data-id="' . $appointment->patient . '"> ' . $patientdetails->name . '</a>';
    ...
    function getMyTodaysAppoinmentList() {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            if ($patient_id == $appointment->patient) {
                $option1 = '<button type="button" class="btn btn-info btn-xs btn_width editbutton" data-toggle="modal" data-id="' . $appointment->id . '"><i ><img alt="" src="uploads/edit-bttn-icon.png" style=" height: 17px;">  ' . lang('edit') . '</i></button>';
                $option2 = '<a class="btn btn-info btn-xs btn_width delete_button" href="appointment/delete?id=' . $appointment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><img alt="" src="uploads/trash-bttn-icon.png"  style=" height: 20px;" > ' . lang('delete') . ' </a>';
                $patientdetails = $this->patient_model->getPatientById($appointment->patient);
                if (!empty($patientdetails)) {
  ...
  public function hospital_otp() {
    $id = $this->input->post('id');
    $doctor_id = $this->input->post('doctor_id');
    $patient_id = $this->input->post('patient_id');
    $doctor= $this->appointment_model->getDoctor($doctor_id);
    $patient= $this->appointment_model->get_patient($patient_id);
    $data = array(
      'status' => $this->input->post('status'),
      ...
      'remarks' => $this->input->post('remarks'),
      'otp' => $this->input->post('verify_code')
    );   
    $result= $this->appointment_model->doctor_appointment_status($data,$id);
  }
  public function specialistFromDepartment()
  {
    ...
    public function doctorFromDepartment()
    {
          $department = $this->input->post('department'); 
          $doctor = $this->appointment_model->doctorFromDepartment($department);
          echo json_encode($doctor);
      }
public function getConsultFee()
{
    $department = $this->input->post('department'); 
    $consultFee = $this->appointment_model->consultFee($department);
    echo json_encode($consultFee);
}
public function getProcedurefee()
{
    $medpro = $this->input->post('medical_procedure'); 
    $procedureFee = $this->appointment_model->procedureFee($medpro);
    echo json_encode($procedureFee);
}
