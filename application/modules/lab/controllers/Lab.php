...
class Lab extends MX_Controller {
  ...
  public function add_lab_report($action,$id) {
    ...
    $this->load->view('add_lab_report_pdf', $data);
    $this->load->view('home/footer');
  }
  public function pdf_report($action,$id) {
    if(!empty($_POST['data'])){
      $pdf_report = base64_decode($_POST['data']);
      $file_name = date('YmdHis').'.'.'pdf';
      file_put_contents( "./uploads/pdf/".$file_name, $pdf_report );
      
      $lab = $this->lab_model->getLabById($id);
      $appointment = $this->lab_model->getappoitment($lab->appointment)[0];
      $patient_details = $this->patient_model->getPatientById($appointment->patient);
      ...
      $pdf_report = base_url()."uploads/pdf/" . $file_name;
      $type = 'Lab Report';
      if($action=='add'){
        $data = array(
          'report' => $lab->report,
          ...
          'doctor' => $doctor,
          'user' => $patient,
          'appoitment_id' => $lab->appointment,
          'patient_name' => $patient_name,
          'patient_phone' => $patient_phone,
          'patient_address' => $patient_address,
          'doctor_name' => $doctor_name,
          'date_string' => date('d-m-Y', $date_string),
          'report_pdf' =>$pdf_report,
          'category'=>$type
        );
        $inserted_id = $this->lab_model->insertScan($data);
        // $url = "https://api.ovalsms.com/api/v2/SendSMS?ApiKey=lkRaw3enwcfvA/FIzdtiMQo7YR9h5AHwCXpX4UKgJNo=&ClientId=83450a2c-0c20-4f50-b1d7-53f5f748b35e&SenderId=DigitalNews&Message=".urlencode("Your medical report for your visit id ".$id." on ".$date_string." at  is now ready. Please down the following link to access your report in MEDAPP mobile app. “https://bit.ly/3GSJ4pz”")."&MobileNumbers=".$data['patient_phone']."&Is_Unicode=true&Is_Flash=false";
        ...
        // curl_close($ch);
        $data = ['phone' => $data['patient_phone'], 'text' => "Your medical report for your visit id ".$id." on ".$date_string." at  is now ready. Please down the following link to access your report in MEDAPP mobile app. “https://bit.ly/3GSJ4pz”"];
  ...
  public function lab() {
    ...
    public function addLab() {
      ...
      if ($this->form_validation->run() == FALSE) {
        ...
        if ($image) {   
          ...
          $img_url = base_url()."uploads/reports/" . $path['file_name'];
        }
        $config1 = array(
        ...
        $report_pdf = $this->upload->do_upload('report_pdf');
        if ($report_pdf) {
          $path = $this->upload->data();
          $pdf_report = base_url()."uploads/pdf/" . $path['file_name'];
        }
        if (empty($id)) {
          $data = array(
            'report' => $report,
          ...
          $id = $this->lab_model->insertLab($data);
          $this->session->set_flashdata('feedback', lang('added'));
          // redirect('lab/add_lab_report/add/'.$id);
          redirect($redirect);
        } else {
          $data = array(
            'report' => $report,
          ...
          $data['report_image'] = $img_url;
          $data['report_pdf'] = $pdf_report;
          $this->lab_model->updateLab($id, $data);
          $this->session->set_flashdata('feedback', lang('updated'));
          redirect($redirect);
          // redirect('lab/add_lab_report/update/'.$id);
        }
      }
    }
    ...
    function delete_scan() {
      if ($this->ion_auth->in_group(array('admin', 'Laboratorist'))) {
        $id = $this->input->get('id');
        $this->lab_model->delete_scan($id);
        $this->session->set_flashdata('feedback', lang('deleted'));
        redirect('lab/scan_report');
      } else {
        redirect('home/permission');
      }
    }
...
// Validating Description Field
      ...
      function scan_invoice() {
        ...
        $id = $this->input->get('id');
        $data['settings'] = $this->settings_model->getSettings();
        $data['lab'] = $this->lab_model->getscan_reportById($id);
        ...
        $this->load->view('home/dashboard'); // just the header file
        $this->load->view('scan_invoice', $data);
        $this->load->view('home/footer'); // just the footer fi
      }
...
// } 
      ...
      function getLab() {
        ...
        foreach ($data['labs'] as $lab) {
          ...
          if ($this->ion_auth->in_group(array('admin', 'Laboratorist', 'Doctor'))) {
            $options1 = ' <a class="btn btn-info btn-xs editbutton" style="padding: 2px 4px;margin-bottom: 1px;" title="' . lang('edit') . '" href="lab?id=' . $lab->id . '"><img alt="" src="uploads/edit-bttn-icon.png" style=" height: 17px;"> ' . lang('') . '</a>';
          } else {
            $options1 = '';
          }

          $options2 = '<a class="btn btn-xs invoicebutton"  title="' . lang('lab') . '" style="color: #fff; padding: 2px 8px;" href="lab/invoice?id=' . $lab->id . '"><i class="fa fa-file"></i> ' . lang('') . '</a>';

          if ($this->ion_auth->in_group(array('admin', 'Doctor', 'Laboratorist'))) {
            $options3 = '<a class="btn btn-info btn-xs delete_button" style="padding: 2px 4px;" title="' . lang('delete') . '" href="lab/delete?id=' . $lab->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><img alt="" src="uploads/trash-bttn-icon.png" style="height:20px;">' . lang('') . '</a>';
          } else {
            $options3 = '';
          }
      ...
      public function scan() {
        ...
        if (!empty($id)) {
          $lab_details = $this->lab_model->getscan_reportById($id);
          if ($lab_details->hospital_id != $this->session->userdata('hospital_id')) {
            redirect('home/permission');
          }
        }
        ...
        if (!empty($id)) {
          $data['lab_single'] = $this->lab_model->getscan_reportById($id);
            // $data['lab_single'] = $this->lab_model->getLabById($id);
          $data['patients'] = $this->patient_model->getPatientById($data['lab_single']->patient);

          $data['doctors'] = $this->doctor_model->getDoctorById($data['lab_single']->doctor);
          $data['appointment_patients'] = $this->patient_model->getPatientappoitment();
          $data['appointments'] = $this->lab_model->getappoitment();
        }
      ...
      public function scan_report() {
        ...
        $data['settings'] = $this->settings_model->getSettings();
        $data['scans'] = $this->lab_model->getscan_report();
        $data['appointments'] = $this->lab_model->getappoitment();
        // var_dump($data['patients']);die;

        if (!empty($id)) {
            $this->load->view('home/dashboard'); // just the header file
            $this->load->view('add_scan_view', $data);
            $this->load->view('home/footer'); // just the header file
          } else {
            $this->load->view('home/dashboard'); // just the header file
            $this->load->view('scan',$data);
      ...
      public function getappoitment($id) {
        $data = $this->lab_model->getappoitment($id);
      ...
      public function addScan() {
        ...
        if ($this->form_validation->run() == FALSE) {
          $this->session->set_flashdata('feedback', lang('please_enter'));
          redirect('lab/scan_report');
        } else {
          if (!empty($p_name)) {
            $data_p = array(
          ...
          if (empty($id)) {
            ...
            $log_data["hospital_id"] = $this->hospital_id;
            $log_data["patient_id"] = $patient;
            $log_data["doctor_id"] = $doctor;
            $log_data["status"] = 'diagnostic';
            $this->patient_model->patient_history_log($log_data);
            // $url = "https://api.ovalsms.com/api/v2/SendSMS?ApiKey=lkRaw3enwcfvA/FIzdtiMQo7YR9h5AHwCXpX4UKgJNo=&ClientId=83450a2c-0c20-4f50-b1d7-53f5f748b35e&SenderId=DigitalNews&Message=".urlencode("Your medical report for your visit id ".$id." on ".$date_string." at  is now ready. Please down the following link to access your report in MEDAPP mobile app. “https://bit.ly/3GSJ4pz”")."&MobileNumbers=".$data['patient_phone']."&Is_Unicode=true&Is_Flash=false";
            ...
            $log_data["status"] = 'diagnostic';
            $this->patient_model->patient_history_log($log_data);
                 // $url = "http://sendsms.designhost.in/index.php/smsapi/httpapi/?uname=medappdynamics&password=free123&sender=MEDZIP&tempid=1607100000000150350&receiver=".$data['patient_phone']."&route=TA&msgtype=1&sms=".urlencode("The report has been sent to your MEDAPPS login.");
            // $url = "https://api.ovalsms.com/api/v2/SendSMS?ApiKey=lkRaw3enwcfvA/FIzdtiMQo7YR9h5AHwCXpX4UKgJNo=&ClientId=83450a2c-0c20-4f50-b1d7-53f5f748b35e&SenderId=DigitalNews&Message=".urlencode("Your medical report for your visit id ".$id." on ".$date_string." at  is now ready. Please down the following link to access your report in MEDAPP mobile app. “https://bit.ly/3GSJ4pz”")."&MobileNumbers=".$data['patient_phone']."&Is_Unicode=true&Is_Flash=false";
...
function getScan() {
  ...
  if ($limit == -1) {
    if (!empty($search)) {
      ...
      $data['labs'] = $this->lab_model->getscan_report();
    }
  } else {
    if (!empty($search)) {
      $data['labs'] = $this->lab_model->getLabByLimitBySearch($limit, $start, $search);
    } else {
      $data['labs'] = $this->lab_model->getScanByLimit($limit, $start);
    }
  }
        //  $data['labs'] = $this->lab_model->getLab();
  ...
  foreach ($data['labs'] as $lab) {
    $date = date('d-m-y', $lab->date);
    if ($this->ion_auth->in_group(array('admin', 'Laboratorist', 'Doctor'))) {
      $options1 = ' <a class="btn btn-info btn-xs editbutton" title="' . lang('edit') . '" href="lab/scan?id=' . $lab->id . '"><i class="fa fa-edit"> </i> ' . lang('') . '</a>';
    } else {
      $options1 = '';
    }

    $options2 = '<a class="btn btn-xs invoicebutton" title="' . lang('lab') . '" style="color: #fff;" href="lab/scan_invoice?id=' . $lab->id . '"><i class="fa fa-file"></i> ' . lang('') . '</a>';

    if ($this->ion_auth->in_group(array('admin'))) {
      $options3 = '<a class="btn btn-info btn-xs delete_button" title="' . lang('delete') . '" href="lab/delete_scan?id=' . $lab->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"></i>' . lang('') . '</a>';
    } else {
      $options3 = '';
    }
    ...
    if (!empty($patient_info)) {
      ...
      $patient_details = $patient_info->address . '</br>' . $patient_info->phone . '</br>';
    } else {
      $patient_details = ' ';
    }
            // $appointment = $this->lab_model->getscan_reportById($lab->id);
            // // var_dump($lab->id);
