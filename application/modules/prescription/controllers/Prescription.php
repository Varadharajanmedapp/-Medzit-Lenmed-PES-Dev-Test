...
class Prescription extends MX_Controller {
    ...
    public function addNewPrescription() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $report_img = $this->upload->do_upload('report_img');
            if ($report_img) {   
              $path = $this->upload->data();
              $data['report_img'] = base_url()."uploads/reports/" . $path['file_name'];
            }
            // $this->prescription_model->updateICD($patient,array('icd_10' => $icd_10));
            if (empty($id)) {
    ...
    function getAppointmentByJasonByDoctor() {
        ...
        foreach ($query as $entry) {
            ...
            $jsonevents[] = array("id" => $entry->id, "text" => $entry->id . ' (' . lang('id') . ': ' . $entry->id . ')');
    ...
    function getAppointmentByJasonByID() {

        $id = $this->input->get('id');

        $query = $this->appointment_model->getAppointmentById($id);
    ...
    function getPrescriptionListByDoctor() {
        ...
        foreach ($data['prescriptions'] as $prescription) {
            ...
            $option2 = '<a class="btn btn-info btn-xs btn_width delete_button" href="prescription/delete?id=' . $prescription->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><img alt="" src="uploads/trash-bttn-icon.png" ></a>';
    ...
    function getPrescriptionList() {
        ...
        foreach ($data['prescriptions'] as $prescription) {
            ...
            $option3 = '<a class="btn btn-info btn-xs btn_width" href="prescription/editPrescription?id=' . $prescription->id . '" data-id="' . $prescription->id . '"><i class="fa fa-edit"></i> ' . lang('edit') . ' ' . lang('prescription') . '</a>';
            $option2 = '<a class="btn btn-info btn-xs btn_width delete_button" href="prescription/delete?id=' . $prescription->id . '&admin=' . $prescription->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><img alt="" src="uploads/trash-bttn-icon.png" style="height:20px;"></a>';
            $options4 = '<a class="btn btn-info btn-xs invoicebutton" title="' . lang('print') . '" style="color: #fff;" href="prescription/viewPrescriptionPrint?id=' . $prescription->id . '"target="_blank"> <i class="fa fa-print"></i> ' . lang('print') . '</a>';
