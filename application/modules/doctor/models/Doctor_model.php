...
class Doctor_model extends CI_model {
    ...
    function insertDoctor($data) {
        ...
        $data3  = $this->hos_location();
        $data2 = array_merge($data, $data1,$data3);
    ...
    function getDoctorInfo($searchTerm) {
        ...
        $users_hos = $fetched_records->result_array();
        $data = array();
        foreach ($users as $user) {
            $data[] = array("id" => $user['id'], "text" => $user['name'] . ' (' . lang('id') . ': ' . $user['id'] . ')');
        }
        foreach ($users_hos as $user) {
            $data[] = array("id" => "hos_".$user['id'], "text" => $user['name'] . ' (' . lang('id') . ': ' . $user['id'] . ')');
        }
    ...
    function getDocWithAddNewOption($searchTerm) {
        ...
        if (!empty($searchTerm)) {
            ...
            $this->db->group_end();
            $this->db->where("(id LIKE '%" . $searchTerm . "%')", NULL, FALSE)
                    ->get();
            $users = $query->result_array();
        } else {
            $this->db->select('*');
            ...
            // $this->db->limit(10);
            $this->db->order_by('id','Desc');
            $fetched_records = $this->db->get('appointment');
            $users = $fetched_records->result_array();
        }
    ...
    public function get_specialist_byDept($department)
    {
