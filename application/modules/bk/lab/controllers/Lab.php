...
class Lab extends MX_Controller {
    ...
    public function addLabView() {
        ...
        $data['templates'] = $this->lab_model->getTemplate();
        $data['settings'] = $this->settings_model->getSettings();
        $data['hos_details'] = $this->settings_model->get_hos_details();
        $data['categories'] = $this->lab_model->getLabCategory();
        // $data['patients'] = $this->patient_model->getPatient();
        // $data['doctors'] = $this->doctor_model->getDoctor();
        $this->load->view('home/dashboard'); // just the header file
...
public function scan() {
      ...
      public function scan_report() {
        ...
        if (!empty($id)) {
            $ssthis->load->view('home/dashboard'); // just the header file
...
// var_dump($asa);die;
            ...
            // var_dump($this->upload->do_upload('report_pdf'));die;
            // var_dump($report_pdf);die;
            if ($report_pdf) {
              $path = $this->upload->data();
...
// Validating Price Field
        $this->form_validation->set_rules('discount', 'Discount', 'trim|min_length[1]|max_length[100]|xss_clean');
        $this->form_validation->set_rules('report_link', 'report_link', 'trim|required|xss_clean');
...
// Adding New Patient
            ...
            if (empty($id)) {
                ...
                $this->lab_model->insertScan($data);
                $inserted_id = $this->db->insert_id();
