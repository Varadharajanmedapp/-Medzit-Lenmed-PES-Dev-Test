...
class Payments extends MX_Controller {
  ...
  function __construct() {
    ...
    if (!$this->ion_auth->in_group(array('superadmin','admin', 'Accountant', 'Receptionist', 'Nurse', 'Laboratorist', 'Doctor'))) {
  ...
  public function payment() {
    ...
    $data['settings'] = $this->settings_model->getSettings();
      ...
      public function addPaymentView() {
        $data = array();
        $data['discount_type'] = $this->payment_model->getDiscountType();
        $data['settings'] = $this->settings_model->getSettings();
        $data['categories'] = $this->payment_model->getPaymentCategory();
        $data['gateway'] = $this->payment_model->getGatewayByName($data['settings']->payment_gateway);
      ...
      public function addPayment() {
        ...
        if (empty($item_selected)) {
          ...
          redirect('payments/addPaymentView');
        } else {
        ...
        if (!empty($item_quantity_array)) {
          foreach ($item_quantity_array as $key => $value) {
            $current_item = $this->payment_model->getPaymentCategoryById($key);
...
// Adding New Patient
          ...
          $sub_total = $amount;
          $discount_type = $this->payment_model->getDiscountType();
          if (!empty($doctor)) {
            ...
            foreach ($all_cat_name as $indiviual_cat_nam) {
              ...
              $d_commission = $this->payment_model->getPaymentCategoryById($indiviual_cat_nam1[0])->d_commission;
          ...
          if (empty($id)) {
            ...
            if ($deposit_type == 'Card') {
              ...
              if ($gateway == 'PayPal') {
                ...
                if ($chargeJson['status'] == 'succeeded') {
                  ...
                  $this->payment_model->insertDeposit($data1);
                  $data_payment = array('amount_received' => $amount_received, 'deposit_type' => $deposit_type);
                  $this->payment_model->updatePayment($inserted_id, $data_payment);
                } else {
                  $this->session->set_flashdata('feedback', lang('transaction_failed'));
                }
                redirect("payments/invoice?id=" . "$inserted_id");
            ...
            if (!empty($deposit_edit_amount)) {
              ...
              foreach ($deposited_edit as $key_deposit => $value_deposit) {
                ...
                $this->payment_model->updateDeposit($key_deposit, $data_deposit);
              }
            }
            ...
            if (!empty($deposit_id->id)) {
              ...
              $this->payment_model->updateDeposit($deposit_id->id, $data1);
            } else {
              $data1 = array(
                'date' => $date,
                'patient' => $patient,
            ...
            redirect("payments/invoice?id=" . "$id");
          }
        }
      }
      ...
      function editPayment() {
        if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist'))) {
          ...
          if (!empty($id)) {
            $payment_details = $this->payment_model->getPaymentById($id);
        ...
        function delete() {
          if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist'))) {
            ...
            $this->payment_model->deleteDepositByInvoiceId($id);
        ...
        public function otPayment() {
          ...
          $data['settings'] = $this->settings_model->getSettings();
          $data['ot_payments'] = $this->payment_model->getOtPayment();
...
// redirect('accountant/add_new'); 
          ...
          if (empty($id)) {
            ...
            $this->payment_model->insertOtPayment($data);
            $inserted_id = $this->db->insert_id();
            $data1 = array(
              'date' => $date,
              'patient' => $patient,
            ...
            $this->session->set_flashdata('feedback', lang('added'));
            redirect("payments/otInvoice?id=" . "$inserted_id");
          } else {
            $a_r_i = $id . '.' . 'ot';
            ...
            $this->payment_model->updateDeposit($deposit_id, $data1);
            $this->payment_model->updateOtPayment($id, $data);
            $this->session->set_flashdata('feedback', lang('updated'));
            redirect("payments/otInvoice?id=" . "$id");
          }
        }
      }
      ...
      function editOtPayment() {
        if ($this->ion_auth->in_group(array('admin', 'Accountant'))) {
          ...
          $data['patients'] = $this->patient_model->getPatient();
          $id = $this->input->get('id');
          $data['ot_payment'] = $this->payment_model->getOtPaymentById($id);
          $data['doctors'] = $this->doctor_model->getDoctor();
            $this->load->view('home/dashboard'); // just the header file
            $this->load->view('add_ot_payment', $data);
            $this->load->view('home/footer'); // just the footer file
      ...
      function otPaymentDelete() {
        if ($this->ion_auth->in_group(array('admin', 'Accountant'))) {
          $id = $this->input->get('id');
          $this->payment_model->deleteOtPayment($id);
          $this->session->set_flashdata('feedback', lang('deleted'));
          redirect('payments/otPayment');
        }
      }
...
// Validating Description Field
        ...
        if ($this->form_validation->run() == FALSE) {
          if (!empty($id)) {
            ...
            redirect('payments/editPaymentCategory?id=' . $id);
          } else {
            $data = array();
            $data['setval'] = 'setval';
                $this->load->view('home/dashboard'); // just the header file
              ...
              if (empty($id)) {
                $this->payment_model->insertPaymentCategory($data);
                $this->session->set_flashdata('feedback', lang('added'));
              } else {
                $this->payment_model->updatePaymentCategory($id, $data);
                $this->session->set_flashdata('feedback', lang('updated'));
              }
              redirect('payments/paymentCategory');
            }
          }
          ...
          function editPaymentCategory() {
            $data = array();
            $id = $this->input->get('id');
            $data['category'] = $this->payment_model->getPaymentCategoryById($id);
        $this->load->view('home/dashboard'); // just the header file
        $this->load->view('add_payment_category', $data);
        $this->load->view('home/footer'); // just the footer file
      }
      ...
      function deletePaymentCategory() {
        $id = $this->input->get('id');
        $this->payment_model->deletePaymentCategory($id);
        redirect('payments/paymentCategory');
      }
      ...
      public function expense() {
        ...
        $data['settings'] = $this->settings_model->getSettings();
        $data['expenses'] = $this->payment_model->getExpense();
      ...
      public function addExpenseView() {
        $data = array();
        $data['settings'] = $this->settings_model->getSettings();
        $data['categories'] = $this->payment_model->getExpenseCategory();
        $this->load->view('home/dashboard'); // just the header file
    ...
    function financialReport() {
        ...
        $data = array();
        $data['payment_categories'] = $this->payment_model->getPaymentCategory();
        $data['expense_categories'] = $this->payment_model->getExpenseCategory();

        $data['payments'] = $this->payment_model->getPaymentByDate($date_from, $date_to);
        $data['ot_payments'] = $this->payment_model->getOtPaymentByDate($date_from, $date_to);
        $data['deposits'] = $this->payment_model->getDepositsByDate($date_from, $date_to);
        $data['expenses'] = $this->payment_model->getExpenseByDate($date_from, $date_to);
...
// Validating Note Field
        ...
        if ($this->form_validation->run() == FALSE) {
          if (!empty($id)) {
            $this->session->set_flashdata('feedback', lang('validation_error'));
            redirect('payments/editExpense?id=' . $id);
          } else {
            $data = array();
            $data['setval'] = 'setval';
            $data['settings'] = $this->settings_model->getSettings();
            $data['categories'] = $this->payment_model->getExpenseCategory();
              ...
              if (empty($id)) {
                $this->payment_model->insertExpense($data);
                $this->session->set_flashdata('feedback', lang('added'));
              } else {
                $this->payment_model->updateExpense($id, $data);
                $this->session->set_flashdata('feedback', lang('updated'));
              }
              redirect('payments/expense');
            }
          }
          ...
          function editExpense() {
            ...
            if (!empty($id)) {
              $expense_details = $this->payment_model->getExpenseById($id);
              if ($expense_details->hospital_id != $this->session->userdata('hospital_id')) {
                redirect('home/permission');
              }
            }

            $data['expense'] = $this->payment_model->getExpenseById($id);
        $this->load->view('home/dashboard'); // just the header file
        $this->load->view('add_expense_view', $data);
        $this->load->view('home/footer'); // just the footer file
      }
      ...
      function deleteExpense() {
        ...
        $this->payment_model->deleteExpense($id);
        redirect('payments/expense');
      }
...
// Validating Description Field
        ...
        if ($this->form_validation->run() == FALSE) {
          if (!empty($id)) {
            $this->session->set_flashdata('feedback', lang('validation_error'));
            redirect('payments/editExpenseCategory?id=' . $id);
          } else {
            $data = array();
            $data['setval'] = 'setval';
                $this->load->view('home/dashboard'); // just the header file
              ...
              if (empty($id)) {
                $this->payment_model->insertExpenseCategory($data);
                $this->session->set_flashdata('feedback', lang('added'));
              } else {
                $this->payment_model->updateExpenseCategory($id, $data);
                $this->session->set_flashdata('feedback', lang('updated'));
              }
              redirect('payments/expenseCategory');
            }
          }
          ...
          function editExpenseCategory() {
            ...
            if (!empty($id)) {
              $expense_category_details = $this->payment_model->getExpenseCategoryById($id);
              if ($expense_category_details->hospital_id != $this->session->userdata('hospital_id')) {
                redirect('home/permission');
              }
            }

            $data['category'] = $this->payment_model->getExpenseCategoryById($id);
        $this->load->view('home/dashboard'); // just the header file
        $this->load->view('add_expense_category', $data);
        $this->load->view('home/footer'); // just the footer file
      }
      ...
      function deleteExpenseCategory() {
        ...
        $this->payment_model->deleteExpenseCategory($id);
        redirect('payments/expenseCategory');
      }
      ...
      function amountReceived() {
        ...
        $this->payment_model->amountReceived($id, $data);
        redirect('payments/invoice?id=' . $id);
      }
      ...
      function otAmountReceived() {
        ...
        $data = array();
        $data = array('amount_received' => $amount_received);
        $this->payment_model->otAmountReceived($id, $data);
        redirect('payments/oTinvoice?id=' . $id);
      }
      ...
      function patientPaymentHistory() {
        ...
        if (!empty($date_from)) {
          $data['payments'] = $this->payment_model->getPaymentByPatientIdByDate($patient, $date_from, $date_to);
          $data['deposits'] = $this->payment_model->getDepositByPatientIdByDate($patient, $date_from, $date_to);
          $data['gateway'] = $this->payment_model->getGatewayByName($data['settings']->payment_gateway);
        } else {
          $data['payments'] = $this->payment_model->getPaymentByPatientId($patient);
          $data['pharmacy_payments'] = $this->pharmacy_model->getPaymentByPatientId($patient);
          $data['ot_payments'] = $this->payment_model->getOtPaymentByPatientId($patient);
          $data['deposits'] = $this->payment_model->getDepositByPatientId($patient);
          $data['gateway'] = $this->payment_model->getGatewayByName($data['settings']->payment_gateway);
        }
...
// Validating Deposited Amount Field
        ...
        if ($this->form_validation->run() == FALSE) {
          redirect('payments/patientPaymentHistory?patient=' . $patient);
        } else {
          $data = array();
          ...
          if (empty($id)) {
            if ($deposit_type == 'Card') {
              ...
              $this->payment_model->insertDeposit($data);
              $this->session->set_flashdata('feedback', lang('added'));
            }
          } else {
            $this->payment_model->updateDeposit($id, $data);

            $amount_received_id = $this->payment_model->getDepositById($id)->amount_received_id;
            if (!empty($amount_received_id)) {
              ...
              $this->payment_model->updatePayment($amount_received_payment_id[0], $data_amount_received);
            }
      ...
      function editDepositByJason() {
        $id = $this->input->get('id');
        $data['deposit'] = $this->payment_model->getDepositById($id);
      ...
      function deleteDeposit() {
        ...
        if (!empty($id)) {
          $deposit_details = $this->payment_model->getDepositById($id);
        ...
        $this->payment_model->deleteDeposit($id);
      ...
      function invoicePatientTotal() {
        ...
        $data['settings'] = $this->settings_model->getSettings();
        $data['discount_type'] = $this->payment_model->getDiscountType();
        $data['payments'] = $this->payment_model->getPaymentByPatientIdByStatus($id);
        $data['ot_payments'] = $this->payment_model->getOtPaymentByPatientIdByStatus($id);
        $data['patient_id'] = $id;
        $this->load->view('home/dashboard'); // just the header file
      ...
      function lastPaidInvoice() {
        ...
        $data['settings'] = $this->settings_model->getSettings();
        $data['discount_type'] = $this->payment_model->getDiscountType();
        $data['payments'] = $this->payment_model->lastPaidInvoice($id);
        $data['ot_payments'] = $this->payment_model->lastOtPaidInvoice($id);
        $data['patient_id'] = $id;
        $this->load->view('home/dashboard'); // just the header file
      ...
      function makePaid() {
        $id = $this->input->get('id');
        $patient_id = $this->payment_model->getPaymentById($id)->patient;
        $data = array();
        $data = array('status' => 'paid');
        $data1 = array();
        $data1 = array('status' => 'paid-last');
        $this->payment_model->makeStatusPaid($id, $patient_id, $data, $data1);
      ...
      function makePaidByPatientIdByStatus() {
        ...
        $this->payment_model->makePaidByPatientIdByStatus($id, $data, $data1);
        $this->session->set_flashdata('feedback', lang('paid'));
        redirect('patient');
      }
      ...
      function makeOtStatusPaid() {
        $id = $this->input->get('id');
        $this->payment_model->makeOtStatusPaid($id);
        $this->session->set_flashdata('feedback', lang('paid'));
        redirect("payments/otInvoice?id=" . "$id");
      }
      ...
      function docComDetails() {
        ...
        $data['doctor'] = $doctor;
        if (!empty($date_from)) {
          $data['payments'] = $this->payment_model->getPaymentByDoctorDate($doctor, $date_from, $date_to);
        } else {
          $data['payments'] = $this->payment_model->getPaymentByDoctor($doctor);
        }
        $data['settings'] = $this->settings_model->getSettings();
        $data['from'] = $this->input->post('date_from');
        $data['to'] = $this->input->post('date_to');
      ...
      function index() {
        $date_from = strtotime(date('m/d/Y', strtotime('-1 year')));
      ...
      function UserActivityReport() {
        ...
        $data['payments'] = $this->payment_model->getPaymentByUserIdByDate($user, $today, $today_last);
        $data['ot_payments'] = $this->payment_model->getOtPaymentByUserIdByDate($user, $today, $today_last);
        $data['deposits'] = $this->payment_model->getDepositByUserIdByDate($user, $today, $today_last);
        $data['day'] = 'Today';
        if (!empty($YESTERDAY_ON)) {
          $today = strtotime($hour . ':00:00');
          $yesterday = strtotime('-1 day', $today);
          $data['day'] = 'Yesterday';
          $data['payments'] = $this->payment_model->getPaymentByUserIdByDate($user, $yesterday, $today);
          $data['ot_payments'] = $this->payment_model->getOtPaymentByUserIdByDate($user, $yesterday, $today);
          $data['deposits'] = $this->payment_model->getDepositByUserIdByDate($user, $yesterday, $today);
        }
        if (!empty($ALL)) {
          $data['day'] = 'All';
          $data['payments'] = $this->payment_model->getPaymentByUserId($user);
          $data['ot_payments'] = $this->payment_model->getOtPaymentByUserId($user);
          $data['deposits'] = $this->payment_model->getDepositByUserId($user);
        }
        $data['settings'] = $this->settings_model->getSettings();
        $this->load->view('home/dashboard'); // just the header file
      ...
      function UserActivityReportDateWise() {
        ...
        $data['payments'] = $this->payment_model->getPaymentByUserIdByDate($user, $date_from, $date_to);
        $data['ot_payments'] = $this->payment_model->getOtPaymentByUserIdByDate($user, $date_from, $date_to);
        $data['deposits'] = $this->payment_model->getDepositByUserIdByDate($user, $date_from, $date_to);
        $data['settings'] = $this->settings_model->getSettings();
        $this->load->view('home/dashboard'); // just the header file
        $this->load->view('user_activity_report', $data);
        $this->load->view('home/footer'); // just the header file
      ...
      function AllUserActivityReport() {
        ...
        if (!empty($user)) {
          ...
          if (empty($user)) {
            ...
            $data['settings'] = $this->settings_model->getSettings();
            $data['payments'] = $this->payment_model->getPaymentByDate($today, $today_last);
            $data['ot_payments'] = $this->payment_model->getOtPaymentByDate($today, $today_last);
            $data['deposits'] = $this->payment_model->getDepositsByDate($today, $today_last);
            $this->load->view('home/dashboard'); // just the header file
        ...
        function getPayment() {
          ...
          if ($limit == -1) {
            if (!empty($search)) {
              $data['payments'] = $this->payment_model->getPaymentBysearch($search);
            } else {
              $data['payments'] = $this->payment_model->getPayment();
            }
          } else {
            if (!empty($search)) {
              $data['payments'] = $this->payment_model->getPaymentByLimitBySearch($limit, $start, $search);
            } else {
              $data['payments'] = $this->payment_model->getPaymentByLimit($limit, $start);
            }
          }
        //  $data['payments'] = $this->payment_model->getPayment();
          ...
          foreach ($data['payments'] as $payment) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Accountant'))) {
              $options1 = ' <a class="btn btn-info btn-xs editbutton" title="' . lang('edit') . '" href="payments/editPayment?id=' . $payment->id . '"><i class="fa fa-edit"> </i> ' . lang('edit') . '</a>';
            }

            $options2 = '<a class="btn btn-info btn-xs invoicebutton" title="' . lang('invoice') . '" style="color: #fff;" href="payments/invoice?id=' . $payment->id . '"><i class="fa fa-file-invoice"></i> ' . lang('invoice') . '</a>';
            $options4 = '<a class="btn btn-info btn-xs invoicebutton" title="' . lang('print') . '" style="color: #fff;" href="payments/printInvoice?id=' . $payment->id . '"target="_blank"> <i class="fa fa-print"></i> ' . lang('print') . '</a>';
            if ($this->ion_auth->in_group(array('admin'))) {
              $options3 = '<a class="btn btn-info btn-xs delete_button" title="' . lang('delete') . '" href="payments/delete?id=' . $payment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"></i> ' . lang('delete') . '</a>';
            }
            ...
            $info[] = array(
              ...
              $settings->currency . '' . $this->payment_model->getDepositAmountByPaymentId($payment->id),
              $settings->currency . '' . ($payment->gross_total - $this->payment_model->getDepositAmountByPaymentId($payment->id)),
              $payment->remarks,
        ...
        function previousInvoice() {
          $id = $this->input->get('id');
          $data1 = $this->payment_model->getFirstRowPaymentById();
          if ($id == $data1->id) {
            $data = $this->payment_model->getLastRowPaymentById();
            redirect('payments/invoice?id=' . $data->id);
          } else {
            for ($id1 = $id - 1; $id1 >= $data1->id; $id1--) {

              $data = $this->payment_model->getPreviousPaymentById($id1);
              if (!empty($data)) {
                redirect('payments/invoice?id=' . $data->id);
                break;
              } elseif ($id1 == $data1->id) {
        ...
        function nextInvoice() {
          ...
          $data1 = $this->payment_model->getLastRowPaymentById();
        //  echo $data1->id;
        //  echo $id;
        //  die();
        //$id1 = $id + 1;
          if ($id == $data1->id) {
            $data = $this->payment_model->getFirstRowPaymentById();
            redirect('payments/invoice?id=' . $data->id);
          } else {
            for ($id1 = $id + 1; $id1 <= $data1->id; $id1++) {

              $data = $this->payment_model->getNextPaymentById($id1);
        ...
        function daily() {
          ...
          $payments = $this->payment_model->getPaymentByDate($first_minute, $last_minute);
          $all_payments = array();
      ...
      function dailyExpense() {
        ...
        $expenses = $this->payment_model->getExpenseByDate($first_minute, $last_minute);
        $all_expenses = array();
      ...
      function getExpense() {
        ...
        if ($limit == -1) {
          if (!empty($search)) {
            $data['expenses'] = $this->payment_model->getExpenseBysearch($search);
          } else {
            $data['expenses'] = $this->payment_model->getExpense();
          }
        } else {
          if (!empty($search)) {
            $data['expenses'] = $this->payment_model->getExpenseByLimitBySearch($limit, $start, $search);
          } else {
            $data['expenses'] = $this->payment_model->getExpenseByLimit($limit, $start);
          }
        }
        //  $data['payments'] = $this->payment_model->getPayment();
        ...
        foreach ($data['expenses'] as $expense) {
          ...
          if ($this->ion_auth->in_group(array('admin'))) {
            $options1 = ' <a class="btn btn-info btn-xs editbutton" title="' . lang('edit') . '" href="payments/editExpense?id=' . $expense->id . '"><i class="fa fa-edit"> </i></a>';
          }

          $options2 = '<a class="btn btn-info btn-xs invoicebutton" title="' . lang('invoice') . '" style="color: #fff;" href="payments/expenseInvoice?id=' . $expense->id . '"><i class="fa fa-file-invoice"></i> </a>';
            //$options4 = '<a class="btn btn-info btn-xs invoicebutton" title="' . lang('print') . '" style="color: #fff;" href="payments/printInvoice?id=' . $payment->id . '"target="_blank"> <i class="fa fa-print"></i> ' . lang('print') . '</a>';
          if ($this->ion_auth->in_group(array('admin'))) {
            $options3 = '<a class="btn btn-info btn-xs delete_button" title="' . lang('delete') . '" href="payments/deleteExpense?id=' . $expense->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"></i> </a>';
          }
